angular.module("umbraco").controller("LeBlender.Dialog.ParameterConfig.Prevalues.Controller",
	function ($scope, assetsService, $http, LeBlenderRequestHelper, dialogService) {
		...
		$scope.init = function () {
			...
			// Store current alias in case it changes and we need to update it in database and xml cache of umbraco
			$scope.oldAlias = $scope.model.value.alias;
		...
		$scope.save = function () {
			...
			if ($scope.model.value) {
				if ($scope.model.value.alias != $scope.oldAlias) {
					$scope.model.value.oldAlias = $scope.oldAlias;
				}
